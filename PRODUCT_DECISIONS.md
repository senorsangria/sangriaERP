...
- Period conflict (SUPERSEDED — see "Production Inventory: incremental entry + per-item upsert" under Production Page Tweaks): originally, if any `OwnInventorySnapshot` existed for `(company, year, month)`, the entire submission was rejected with an error linking to the snapshots page, and the user had to delete existing rows and re-enter from scratch (no partial-period overlay). This coarse month block was removed — entry is now incremental and editable.
...
### Group forecast + PO generation consolidated into the Forecast tab

**Outcome:** the standalone `distributor_group_forecast` page is **retired**. Distributor *and* group forecasting, plus PO generation for both, now live entirely in the Forecast tab. Delivered in two parts (part 1 = render in-tab; part 2 = unify the modal + retire the page).

- **Param scheme:** the Forecast tab reads `?forecast_distributor=N` (single) or `?forecast_group=N` (group). The `distributor_list` view branches on which is present; they are **mutually exclusive — if both are present, the group takes precedence** (the distributor branch is skipped). Group mode sets `forecast_group` + `primary_distributor` context and leaves `forecast_distributor` as `None`; the template branches on `forecast_group`.
- **View logic (part 1):** the group branch performs the member `po_additions` → `compute_group_forecast` → `generate_projected_orders(primary, …)` (orders only when `alignment_status == 'ok'`) — the logic the standalone view used to do.
- **Three group states render in-tab:** `ok` → the forecast grid (near-identical to the single grid; group-specific snapshot tooltip "Aggregated on-hand inventory across all group members"); `misaligned` / `no_data` → the alignment panel. The misaligned/no_data markup is a **reusable partial** `templates/distribution/_group_alignment_panel.html` (reads `forecast_result.alignment_status` / `.alignment_errors` / `.message`).
- **Dropdown:** the group `<option>`'s `data-url` points to `?tab=forecast&forecast_group=N`, so selecting a group stays in the tab. The shared change-handler is generic (navigates to `data-url`). The single-mode "part of a group" banner's "View Group Forecast →" link was also repointed to the tab URL. Single-distributor rendering is **unchanged**.

#### Unified `#poModal` for group PO generation (part 2)

- **One modal, two modes.** The Forecast tab's `#poModal` generates POs for both individual distributors and groups. Group mode **reuses the existing runtime override hook** `window.openDistributorPoModal(dataUrl, year, month, saveUrl, deleteUrlBase, suggestPattern, isGroup)` (the same hook the Distributor POs tab already uses to repoint endpoints) — group order-cell buttons carry the group endpoints (`distributor_group_orders_modal_data` / `distributor_group_po_save` / `distributor_group_po_suggest`) and `data-is-group="1"`; **delete targets the primary's** single endpoint (`/distributors/<primary_pk>/po/`) because a group PO *is* the primary's PO. The 7th `isGroup` arg defaults undefined → single mode, so existing 6-arg callers are unaffected.
- **The three group-only deltas added to the modal JS** (each was absent from the tab modal and would silently break group generation if missed):
  1. **`primary_distributor` data key** — `openModal` reads `data.primary_distributor || data.distributor`; group payload returns the former, single the latter (else `.order_quantity_unit` throws `undefined`).
  2. **Read-only member-PO tabs** — `saved_orders` carry `is_primary` / `distributor_name` / `distributor_pk`; a non-primary member PO (`_state.isGroup && savedPo && !savedPo.is_primary`) renders read-only (labeled by distributor name, with a link to that member's own forecast) via the ported `buildReadOnlyTabContent`. `removeTab` renumbers only editable tabs; `saveAllOrders` skips read-only tabs. Inert in single mode (`is_primary` is never false there).
  3. **Group info banner** — `#po-modal-group-banner` shows "New orders will be placed for **[primary]** (the primary of [group])" only in group mode, populated from the response's `group.name` / `primary_distributor.name`.
...
- **Production Forecast PO creation returns to the Forecast grid** (refreshed), not the Inventory tab. A PO entered from the Forecast grid opens the PO modal in `mode: 'month'`; on a successful save the modal JS in `production_home.html` now navigates to `?tab=forecast` (a single-PO edit opened from the Production POs tab, `mode: 'single'`, returns to `?tab=production_pos`). Previously the success handler called `window.location.reload()`, which — because tab switches are client-side only and the URL has no `?tab=` after a JS switch — reloaded the page on the **default Inventory tab**, dumping the user away from the forecast grid they came from. The forecast grid needs no per-selection param (it is computed company-wide), so `?tab=forecast` alone fully re-renders it.
- **Production Inventory: incremental entry + per-item upsert (edit in place).** Inventory for a month can now be entered incrementally and edited, replacing the original all-or-nothing month block. Changes (`production_inventory_upload` in `apps/production/views.py`, `inventory_upload.html`):
  - **Month block removed.** The coarse `if OwnInventorySnapshot.objects.filter(company, year, month).exists(): …"already exists, delete it first"` guard was deleted. It was *stricter than the DB constraint* (the real guard is the per-`(company, item, year, month)` `unique_together`), and was the sole cause of the partial-snapshot friction. The forecast already reads snapshots per-item, so a month with only some items filled was always a supported state.
  - **Pre-fill on period select.** Picking a Year + Month reloads the entry form via `GET ?year=&month=`; the view pre-fills each item input with its existing `quantity_cases` for that period (formatted with `_format_quantity_cases`, matching the list display) and pre-selects the period dropdowns. The form reads as "enter or update this month's inventory."
  - **Upsert on save.** The create-only loop is now `OwnInventorySnapshot.objects.update_or_create(company, item, year, month, defaults={quantity_cases, updated_by}, create_defaults={…, created_by})` per filled item, inside the existing `transaction.atomic()`. Existing rows are updated in place (no duplicates); new items are created. `created_by` is set only on creation (so it is preserved on update); `updated_by` is stamped on both.
  - **Blank = leave unchanged (not delete).** A previously-filled item submitted blank is skipped, leaving its existing row untouched. Deletion remains explicit via the Inventory tab's per-row delete checkboxes — treating a cleared field as a delete would make it too easy to wipe a value accidentally.
  - All other validations are unchanged (decimal parsing, `MinValueValidator(0)`, negative/non-numeric rejects the whole atomic submission, all-blank → info message). After save, still redirects to `?tab=inventory`.
- **`OwnInventorySnapshot.updated_by` added** (FK → `core.User`, `SET_NULL`, nullable, `related_name='own_snapshots_updated'`; migration `production/0003`, a clean nullable `AddField`). Records who last created or updated each value. Surfaced subtly on the Inventory tab list as a muted "Updated by <name>" sub-line under the Entered date (the list query adds `select_related('updated_by')`; `_build_snapshot_rows` emits `updated_by_display` = full name → email → username).
...
- **Management listing: clicking the distributor name opens the edit form.** In `distributor_list.html` the per-row distributor *name* link now targets `distributor_edit` (carrying `?next=` back to the listing), instead of `distributor_detail` (the retailers/accounts list, which was low value as the primary click target). The **redundant per-row Edit button was removed** — its function is now served by clicking the name. The per-row **View** button (→ `distributor_detail`) and the Deactivate/Reactivate toggle are unchanged, so the detail page still has an entry point (the View button) and was **not** retired.
- **Edit-form back-link returns to the distributor listing.** The back-link on `distributor_edit.html` now points to `distributor_list` and uses the app-standard **"Back to Distributors"** label (matching `distributor_detail.html`, `account_detail.html`, etc.), instead of pointing to `distributor_detail` with a **"Back to [Distributor Name]"** label. This sends the user back to where they came from (the listing, now the entry point) and follows the convention `<i class="bi bi-arrow-left"></i>Back to [Section]`.
