...
| **R2** | Retire the event_import tool — delete the app, its 8 endpoints, and `can_run_historical_event_import`. This **is** the fix for the one confirmed cross-tenant read+write breach (02 T1, CRITICAL), the single-tenant-hardwired tool surface (02 T9), and the permission cleanup (03 P12). Resolution per PRODUCT_DECISIONS: retire, don't fix | 02 T1/T9, 03 P12 | CRITICAL (T1) | S | env-independent | Must land before any external tenant credential is issued. If retirement slips, the S-cost stopgap is validating posted pks against the candidate set |
...
| **R7** | Decide the saas_admin access model (recommendation on the table: saas_admin = Django-admin operator only; remove the app-side all-tenant account search branch) | 02 T5, 03 P6 | MEDIUM | S | env-independent | Decision is the product owner's — see Scope Notes Q3. Implemented inside R1's decorator |
...
| **R21** | Redundant index drops on sales_salesrecord, per production scan counts: `company_id` single = clear drop; `account_id` single = defensible drop (see Scope Notes Q2); `(item, sale_date)` pair = review after R26. Execution wrinkle: Django auto-recreates FK indexes on a naive RemoveIndex — making drops stick is an execution detail | 01 D16, 04 F7, 00 | LOW (01) / MEDIUM-QW (04) | S | **revised-by-00** | One no-downtime migration per drop |
| **R22** | Event list payload rework: render only the requested tab, paginate/lazy-load the Past tab (3.54 MB measured today; 98.6% of rows are imported history). Absorbs the sidebar fan-out (04 F8); the mobile/desktop double-render is the follow-up polish (pairs with R39/R40) | 04 F1 (+F8), 05 T-3 | HIGH | S-M | dev-measured | Day-one reality for any tenant importing history |
...
| ID | Item | Source | Sev | Cost | Prod status | Notes / dependencies |
|---|---|---|---|---|---|---|
| **R23** | SalesRecord grain decision — production bounded this to **149 groups (max 3 rows) out of 110,018**, all intra-batch, 99.86% naturally unique. Eyeball a sample of the 149 (Scope Notes Q1), then either add a line discriminator / accept a documented surrogate grain (if legitimate multi-line sales) or dedupe + add a uniqueness constraint (if import artifacts) | 01 D1, backlog #1, 00 | HIGH → bounded | S-M (was open-ended M) | **revised-by-00** | Before COGS, not a blocker. The reconcile key for exports depends on it |
| **R24** | Stop the AccountItemPriceHistory write path now; drop the table later. Production showed it is not just dead but actively writing misleading data (logged per import touch, not per price change; never matches current_price). Do **not** copy this pattern for the planned ending-inventory feature — OwnInventorySnapshot is the right precedent | 01 D9, backlog #7, 00 | MEDIUM, strengthened | S | **revised-by-00** | 00 recommends stopping the write immediately; gated here because COGS pricing is the next work that would touch it |
...
> **PROPOSED — execution order is decided by the product owner, not this document.** Slices are small and independently shippable per the Phase 3 model. Sequencing honors: the PAT rotation (R15/O5) is already done; the Django LTS upgrade comes early so all later changes land on a supported framework; error visibility is the cheapest high-value fix; CI precedes the heavy refactoring slices because it protects everything after it.

0. ~~PAT rotation (R15)~~ — **completed.**
1. **R13 — Error visibility** (logging + Sentry + uptime). Cheapest high-value fix; every slice after this lands with eyes on production.
2. **R12 — SECURE_* settings block.** One small PR closes the live cookie-security gap.
3. **R11 — Django 5.2 LTS + Pillow + Dependabot.** Early, so everything below lands on a supported framework; the existing test suite verifies it.
4. **R17 (S-half) — CI + deploy runbook.** Makes the 1,337 tests a deploy gate before any heavy refactoring begins.
5. **R16 — /healthz + /ops/status.** Verifies in production that slices 2–3 actually took effect.
6. **R18 — gunicorn workers.** Removes the one-request-at-a-time choke.
7. **R14 — Backups**: verify plan, nightly dump to R2, restore drill, R2 versioning. Must precede any data-rewriting migration.
8. **R2 — Retire event_import.** Closes the only CRITICAL finding; gates issuing any external credential.
9. **R3 + R4 — Small scoping patches** (event-query company terms; reports `?items=` validation).
10. **R1 (slice 1) — Shared fixtures → two-company harness + lint**, wired into CI. The audit census is the test spec.
11. **R1 (slice 2) + R5 + R6 + R7 — Decorator rollout begins** (new code mandatory, backfill incremental), enforcing the account permissions, with the operator-policy and saas_admin decisions landed in the same mechanism. Scoped managers adopt app-by-app from here on.
12. **R20 — Import AccountItem bulk fix.** Before tenant #1's first big history import.
13. **R21 — Index drops** (the clear one now; the judgment one per Scope Notes Q2).
14. **R22 — Event-list rework** (requested-tab render + Past-tab pagination).
15. **R8 — CoPacker UI.** Unblocks production features for tenants.
16. **R9 + R10 — provision_tenant + onboarding runbook + seed-data guards.**
17. **R19 — DEPLOYMENT.md truth-up + .env.example.**
18. **R17 (M-half) — Staging service**, used to rehearse the Bucket-2 data-rewriting migrations before tenant #1.

---

## SCOPE NOTES — decisions the roadmap needs from the product owner

1. **The 149 SalesRecord grain rows (R23):** are same-day duplicate (account, item, date) rows legitimate multi-line sales or import artifacts? Recommend eyeballing a sample of the 149 before choosing: line discriminator / documented surrogate grain (if legitimate) vs dedupe + uniqueness constraint (if artifacts). This decision defines the reconcile key the COGS exports use.
2. **The `account_id` single-index drop (R21):** production shows it heavily used (627k scans) but fully covered by the composite's leading column. Dropping trades a marginally larger read scan on the hottest read path for one fewer index maintained on every import write. Take the write-path win, or keep it until after R26 changes the plan space?
3. **saas_admin operating model (R7):** confirm the recommendation — saas_admin operates via Django admin only, and the app-side all-tenant account search (the only by-design cross-tenant read path) is removed?
...
6. **event_import retirement scope (R2):** confirm the tool is gone for good (vs. mothballed for a future rebuilt importer) — this determines whether its app/migrations are deleted outright.
