...
### Group forecast rendered in the Forecast tab (consolidation — part 1)

- **Goal:** retire the standalone `distributor_group_forecast` page by rendering a group's forecast **inside** the existing Forecast tab. Part 1 (this change) makes the group forecast *visible* in-tab; part 2 unifies the PO modal and removes the standalone page.
- **Param scheme:** the Forecast tab reads `?forecast_distributor=N` (single) or the new `?forecast_group=N` (group). The `distributor_list` view branches on which is present; they are **mutually exclusive — if both are present, the group takes precedence** (the distributor branch is skipped). Group mode sets `forecast_group` + `primary_distributor` context and leaves `forecast_distributor` as `None`; the template branches on `forecast_group`.
- **View logic ported, not moved:** the group branch mirrors the standalone `distributor_group_forecast` view body (member `po_additions` → `compute_group_forecast` → `generate_projected_orders(primary, …)` only when `alignment_status == 'ok'`). The standalone view/URL/template are **unchanged** and remain as a temporary fallback until part 2.
- **Three group states render in-tab:** `ok` → the forecast grid (near-identical to the single grid; group-specific snapshot tooltip "Aggregated on-hand inventory across all group members"); `misaligned` / `no_data` → the alignment panel. The misaligned/no_data markup was **extracted to a reusable partial** `templates/distribution/_group_alignment_panel.html` (reads `forecast_result.alignment_status` / `.alignment_errors` / `.message`).
- **Order generation in part 1 is read-only in-tab:** the group grid shows saved-PO counts as static badges plus an info note linking to the standalone group page for *generation*. The tab's `#poModal` JS is **not** group-aware yet (no modal JS changed in part 1) — that, and the standalone page's retirement, are part 2.
- **Dropdown:** the group `<option>`'s `data-url` now points to `?tab=forecast&forecast_group=N` (was the standalone URL), so selecting a group stays in the tab. The shared change-handler is generic (navigates to `data-url`) and needed no change. Single-distributor rendering is **unchanged**.
